			...
			<div class="ec-content-wrapper">
				<div class="content">
					<div class="breadcrumb-wrapper d-flex align-items-center justify-content-between">
						...
						<div>
							<a href="quan_tri.php?p=them_tin_dichvu" class="btn btn-primary"> Thêm dịch vụ</a>
						</div>
					</div>
					<div class="row">
						<?php
						...
						// Count total rows in tin_dichvu
						...
						if (isset($_REQUEST['list_id']) && $_REQUEST['list_id'] != '') {
							...
							for ($i = 0; $i < count($del_pic_id); $i++) {
								$sql = "SELECT * FROM tin_dichvu WHERE id = '" . mysqli_real_escape_string($link, $del_pic_id[$i]) . "'";
								$result = mysqli_query($link, $sql);
							}
							$delete_query = 'DELETE FROM tin_dichvu WHERE id IN(' . mysqli_real_escape_string($link, $list_id) . ')';
							mysqli_query($link, $delete_query);
							?>
							<script>
								window.location = "quan_tri.php?p=ds_tin_dichvu&page=<?php echo $_GET['page']; ?>"
							</script>
							<?php
						}
						...
						if (isset($_REQUEST['noibat']) && $_REQUEST['noibat'] != '') {
							...
							$id = $_GET['id'];
							if ($noibat == 0) {
								mysqli_query($link, "update tin_dichvu set noibat=1 where id=" . $id . "");
							}
							if ($noibat == 1) {
								mysqli_query($link, "update tin_dichvu set noibat=0 where id=" . $id . "");
							}
							?>
							<script>
								window.location = "quan_tri.php?p=ds_tin_dichvu&page=<?php echo $_GET['page']; ?>"
						...
						if (isset($_REQUEST['moi']) && $_REQUEST['moi'] != '') {
							...
							$id = $_GET['id'];
							if ($noibat == 0) {
								mysqli_query($link, "update tin_dichvu set moi=1 where id=" . $id . "");
							}
							if ($noibat == 1) {
								mysqli_query($link, "update tin_dichvu set moi=0 where id=" . $id . "");
							}
							?>
							<script>
								window.location = "quan_tri.php?p=ds_tin_dichvu&page=<?php echo $_GET['page']; ?>"
						...
						if (isset($_REQUEST['km']) && $_REQUEST['km'] != '') {
							...
							$id = $_GET['id'];
							if ($noibat == 0) {
								mysqli_query($link, "update tin_dichvu set km=1 where id=" . $id . "");
							}
							if ($noibat == 1) {
								mysqli_query($link, "update tin_dichvu set km=0 where id=" . $id . "");
							}
							?>
							<script>
								window.location = "quan_tri.php?p=ds_tin_dichvu&page=<?php echo $_GET['page']; ?>"
						...
						if (isset($_REQUEST['chay']) && $_REQUEST['chay'] != '') {
							...
							$id = $_GET['id'];
							if ($noibat == 0) {
								mysqli_query($link, "update tin_dichvu set chay=1 where id=" . $id . "");
							}
							if ($noibat == 1) {
								mysqli_query($link, "update tin_dichvu set chay=0 where id=" . $id . "");
							}
							?>
							<script>
								window.location = "quan_tri.php?p=ds_tin_dichvu&page=<?php echo $_GET['page']; ?>"
						...
						<script language="javascript">
							function delete_all_chosen() {
								...
								window.location = "quan_tri.php?p=ds_tin_dichvu&list_id=" + list_id;
								return true;
							}
						</script>
						...
						class pager
						{
							...
							function pageList($curpage, $pages)
							{
								...
								if (($curpage != 1) && ($curpage)) {
									$page_list .= "<a href=\"" . $_SERVER['PHP_SELF'] . "?p=ds_tin_dichvu&page=1\" title=\"Trang đầu\" style='margin:4px;'> << </a>";
								}
								if ($curpage - 1 > 0) {
												...
												while ($row = mysqli_fetch_object($result)) {
													...
													<tr>
														...
														<td>
															<?php if ($moi == 0) { ?>
																<a
																	href="quan_tri.php?p=ds_tin_dichvu&moi=0&id=<?php echo $id; ?>&page=<?php echo $_GET['page']; ?>">No</a>
															<?php } else { ?>
																<a
																	href="quan_tri.php?p=ds_tin_dichvu&moi=1&id=<?php echo $id; ?>&page=<?php echo $_GET['page']; ?>">Active</a>
															<?php } ?>
														</td>
														...
														<td>
															<a href="quan_tri.php?p=xoa_tin_dichvu&id=<?= $id ?>"
														...
														<td>
															<a
																href="quan_tri.php?p=sua_tin_dichvu&id=<?= $id ?>&thuocloai=1&page=<?= $_GET['page'] ?>">
