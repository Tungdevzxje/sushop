        ...
        <div class="content">
          ...
          <div class="row">
            ...
            function upload($thuocloai, $tentaptin, $noidung, $id, $url)
            {
              ...
              $truyvan = "update  loai_tin_dichvu set `thuocloai` = '$thuocloai' , `thuocloai_en` = '' , `hinhanh` = '$tentaptin' , `logo` = '', `noidung` = '$noidung' , `name_url` = '$url' , `noidung_en` = '' where id='$id'";

              $ketqua_truyvan = mysqli_query($link, $truyvan);
              echo "<script>window.location='quan_tri.php?p=danhsach_loai_tindichvu&page=" . $_GET["page"] . "'</script>";
              if ($ketqua_truyvan) {
            ...
            include('db.php');
            $id = $_REQUEST["id"];
            $result = mysqli_query($link, "SELECT * FROM loai_tin_dichvu where id like '$id' ");
